  ...
  server.tool(
    ...
    "Fetch a Jira story by issue key. Returns summary, description, acceptance criteria, status, labels, assignee, and all custom fields.",
    {
      issueKey: z
        ...
        .min(1)
        .describe(
          "Jira issue key in the format PROJECT-NNN (e.g. SFDC-123)"
        ),
    },
    async ({ issueKey }) => {
  ...
  server.tool(
    ...
    "Transition a Jira story to a new status. The available transitions depend on the current workflow configuration. Common statuses: 'To Do', 'In Progress', 'In Review', 'Done', 'Blocked'.",
    {
      ...
      targetStatus: z
        ...
        .min(1)
        .describe(
          "The target workflow status name (e.g. 'In Progress', 'Done', 'Blocked'). Must match an available transition exactly."
        ),
    },
    async ({ issueKey, targetStatus }) => {
      ...
      try {
        ...
        return fail(
          `Failed to update status for ${issueKey}`,
  ...
  server.tool(
    ...
    "Post a comment to a Jira story. Used by the AI agent to document deployment results, ask for clarification, or provide status updates. Supports plain text with paragraphs separated by blank lines.",
    {
      ...
      body: z
        ...
        .min(1)
        .describe(
          "Comment text. Use double newlines to separate paragraphs. Supports plain text — rich formatting will be preserved as ADF."
        ),
    },
    async ({ issueKey, body }) => {
      ...
      try {
        ...
        return fail(
          `Failed to post comment to ${issueKey}`,
          extractErrorMessage(err)
        );
      }
    }
  ...
  server.tool(
    ...
    "Search Jira stories using JQL (Jira Query Language). Returns matching issues with full details. Example JQL: 'label = \"sf-config\" AND status = \"To Do\" AND project = SFDC ORDER BY created DESC'",
    {
      jql: z
        ...
        .min(1)
        .describe(
          "JQL query string. Examples: 'project = SFDC AND status = \"To Do\"', 'label = \"sf-config\" AND assignee = currentUser()', 'issuetype = Story AND sprint in openSprints()'"
        ),
      maxResults: z
        .number()
        .int()
